...
void get_from_client(char *content, int sock) {
    ...
    while(loop) {
        ...
        send_to_socket(temp_buff, sock, 1, "ack");
    }
}
...
void send_text(char *encrypted_text, int socket_dest) {
    ...
    send_to_socket(tmp_buff, socket_dest, 1, "exit");
 ...
 * Function: decrypt_it
 * -------------------------
 * Takes an encrpyted character and decrypts using the key character passed in.
 *
 * Parameters:
 * -------------------------
 * encrypted: the encrypted char to be decrypted.
 * key: the key used to decrypt.
 *
 * Returns:
 * -------------------------
 * decrypted: the resulting decrypted character
 */
char decrypt_it(char encrypted, char key) {
    char decrypted;
    int letter_val = letter_to_int(encrypted);
    int key_val = letter_to_int(key);
    int result = (letter_val - key_val) % mod_val;
    if (result < 0) {
        result += mod_val;
    }
    if (letter_val == s) {
        ...
        decrypted = int_to_letter(result);
    }
    return decrypted;
}
 ...
 * Function: decode
 * -------------------------
 * Using the key, decrypts the raw text char by char, and stores the decrypted text in result.
 *
 * Parameters:
 * -------------------------
 * raw_text: the text to be decrypted
 * key: the key used to decrypt the text
 * result: the resulting decrypted text
 *
 * Returns:
 * -------------------------
 * void
 */
void decode(char *encrypted_text, char *key, char *result) {
    int counter = 0;
    while (encrypted_text[counter] != '\0') {
        if (encrypted_text[counter] != '\n') {
            result[counter] = decrypt_it(encrypted_text[counter], key[counter]);
        } else {
            result[counter] = '\0';
        }
        counter++;
...
/*
 * Function: get_decrypt_send
 * -------------------------
 * Gets the text and key files from the client, decrypts the text, and sends the decrypted text back to the client.
 *
 * Parameters:
 * -------------------------
 ...
 * void
 */
void get_decrypt_send(int sock) {
    char text[MAX], key[MAX], decrypted[MAX];
    get_from_client(text, sock);
    get_from_client(key, sock);
    decode(text, key, decrypted);
    send_text(decrypted, sock);
}
...
int main(int argc, char *argv[]) {
    ...
    // Check usage & args
    if (argc < 2) {
        fprintf(stderr,"USAGE: ./dec_server port_number\n");
        exit(1);
    }
    ...
    while(true) {
        ...
        if (spawnPid == -1) {
            ...
            get_decrypt_send(client_socket);
            close(client_socket);
            exit(0);
        }
